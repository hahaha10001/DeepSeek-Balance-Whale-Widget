...
const DARK_IMAGE_CANDIDATES = [
  path.join(PACKAGE_ROOT, 'assets', 'DSniang2.png'),
]
...
var IMG_DARK_URL = '/dsh-whale/image-dark.png?v=2'
...
function applyThemeImage() {
  img.src = isDarkTheme() ? IMG_DARK_URL : IMG_URL
}
applyThemeImage()
if (typeof MutationObserver !== 'undefined') {
  var themeObserver = new MutationObserver(applyThemeImage)
...
function applyTheme() {
  ...
  // 本地适配：让深色鲸鱼图片（PR#12 机制）跟随挂件主题开关
  try {
...
function apply(ctx) {
    ...
    function loadImage() {
      if (!imageBytes) {
        imageBytes = readImage(IMAGE_CANDIDATES)
        if (!imageBytes) throw new Error('whale image not found')
      }
      return imageBytes
    }
    ...
    function loadDarkImage() {
      if (!darkImageBytes) {
        darkImageBytes = readImage(DARK_IMAGE_CANDIDATES)
        if (!darkImageBytes) throw new Error('whale dark image not found')
      }
      return darkImageBytes
    }
    ...
    disposers.push(ctx.webServer.register({
      kind: 'exact',
      path: '/dsh-whale/image-dark.png',
      handler: (req, res) => {
        try {
          const bytes = loadDarkImage()
          res.writeHead(200, {
            'Content-Type': 'image/png',
            'Cache-Control': 'no-store',
            'Content-Length': String(bytes.length),
          ...
          res.writeHead(404, { 'Content-Type': 'text/plain; charset=utf-8' })
          res.end('whale dark image unavailable: ' + String((err && err.message) || err))
        }
      },
    }))
